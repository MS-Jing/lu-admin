...
/**
 * @author luojing
 * @since 2024/11/26 18:24
 */
@Getter
@Setter
@EqualsAndHashCode(callSuper = true)
public class ExportResultInfo extends AbstractTemplateInfo {

    public ExportResultInfo(GenTemplateInfo genTemplateInfo) {
        super(genTemplateInfo);
        Set<String> packages = new HashSet<>();
        List<FieldInfo> fieldInfoList = genTemplateInfo.getFieldInfoList();
        for (FieldInfo fieldInfo : fieldInfoList) {
            if (!fieldInfo.getShowInExport()) {
                continue;
            }
        ...
        String className = genTemplateInfo.getEntity().getClassName() + GenConstant.exportResultSuffix;
    ...
    public String getTemplate() {
        return GenConstant.exportResultTemplate;
    }
}
